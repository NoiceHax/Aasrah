...
import { PushOptIn } from "@/components/portal/push-optin";
...
/** Shared notification center used across all portals. */
    ...
    <div className="space-y-3 p-margin-mobile md:p-margin-desktop">
      ...
      {isLoading ? (
        ...
        data.items.map((n) => {
          ...
          return (
            <Card key={n.id} className={`flex items-start gap-3 p-stack-md ${n.is_read ? "opacity-60" : ""}`}>
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-label-sm text-on-surface-variant">{new Date(n.created_at).toLocaleString()}</p>
              </div>
              {!n.is_read && (
                <button
                  type="button"
